...
interface UnbilledRevenueFormData {
  voucherDate: string;
  customerName: string;
...
export default function AddUnbilledRevenue() {
  ...
  const [formData, setFormData] = useState<UnbilledRevenueFormData>({
  ...
  const contractOptions = [
    { value: "contract1", label: "CON-2024-001" },
    { value: "contract2", label: "CON-2024-002" },
    { value: "contract3", label: "CON-2024-003" },
    { value: "contract4", label: "CON-2024-004" },
    { value: "contract5", label: "CON-2024-005" }
  ];
  ...
  const handleInputChange = (field: keyof UnbilledRevenueFormData, value: string) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const addLineItem = () => {
    ...
    const newLineItem: LineItem = {
      ...
      unbilledAmount: currentLineItem.unbilledAmount
    };
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.log('Unbilled revenue data submitted for approval:', formData);
      
      // Navigate back to unbilled revenue list
      navigate('/services/accounts-receivable/completed-unbilled-revenue');
    } catch (error) {
      console.error('Error submitting unbilled revenue:', error);
    } finally {
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      console.log('Unbilled revenue saved as draft:', formData);
      
      // Navigate back to unbilled revenue list
      navigate('/services/accounts-receivable/draft-unbilled-revenue');
    } catch (error) {
  ...
  const calculateAmounts = (quantity: string, rate: string, igstRate: string, cgstRate: string, sgstRate: string) => {
    ...
    const unbilledAmount = taxableValue + igstAmount + cgstAmount + sgstAmount;
  ...
  return (
    <div>
      <PageMeta
        title={pageMetaTitle}
        description="Accounts Receivable - Add New Unbilled Revenue"
      />
      <PageBreadcrumb pageTitle="Accounts Receivable - Add New Unbilled Revenue" />
      
      <div className="min-h-screen rounded-2xl border border-gray-200 bg-white px-5 py-7 dark:border-gray-800 dark:bg-white/[0.03] xl:px-10 xl:py-12">
        <div className="mx-auto w-full max-w-[1000px]">
          <div className="mb-8 text-center">
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
              Add New Unbilled Revenue
            </h1>
            <p className="mt-2 text-gray-600 dark:text-gray-400">
              Create a new unbilled revenue entry for your client
            </p>
          </div>

          <Form onSubmit={handleSubmit} className="space-y-6">
            {/* Unbilled Revenue Header */}
            <div className="rounded-lg border border-gray-200 bg-gray-50 p-6 dark:border-gray-700 dark:bg-gray-800">
              <h3 className="mb-4 text-lg font-semibold text-gray-900 dark:text-white">
                Unbilled Revenue Details
              </h3>
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                  ...
                  <DatePicker
                    ...
                    onChange={(dates, currentDateString) => {
                      ...
                      if (selectedDate >= today) {
                        handleInputChange('voucherDate', currentDateString);
                      } else {
                        alert('Voucher date cannot be backdated. Please select today or a future date.');
                      }
                    }}
                  />
                </div>
                ...
                <div>
                  ...
                  <Select
                    ...
                    defaultValue={formData.contractReference}
                    onChange={(value) => handleInputChange('contractReference', value)}
                    className="dark:bg-gray-900"
                  />
                </div>
              ...
              <div className="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                    ...
                    <Select
                      ...
                      onChange={(value) => {
                        ...
                        setCurrentLineItem(prev => ({
                          ...
                          unbilledAmount: amounts.unbilledAmount
                        }));
                      }}
                  ...
                  <div>
                    ...
                    <Input
                      type="text"
                      placeholder="Total unbilled amount"
                      value={currentLineItem.unbilledAmount}
                      onChange={(e) => {
                        setCurrentLineItem(prev => ({
                          ...prev,
                          unbilledAmount: e.target.value
                        }));
                      }}
                      disabled
                    />
                      ...
                      <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                        {formData.lineItems.map((item, index) => (
                          <TableRow key={item.id}>
                            ...
                            <TableCell className="px-5 py-4 text-start">
                              <span className="text-gray-800 text-theme-sm dark:text-white/90">
                                {item.unbilledAmount || 'Calculated'}
                              </span>
                            </TableCell>
                            <TableCell className="px-5 py-4 text-start">
                              <Button
